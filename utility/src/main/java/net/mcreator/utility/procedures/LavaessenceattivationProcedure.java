...
public class LavaessenceattivationProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		} else if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(UtilityModEnchantments.WIND_ESSENCE.get()) >= 1) {
			if (Mth.nextInt(RandomSource.create(), 1, 3) == 2) {
		...
		} else if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(UtilityModEnchantments.SHOCKESSENCE.get()) >= 1) {
			if (Mth.nextInt(RandomSource.create(), 1, 3) == 2) {
				{
					Entity _ent = entity;
		...
		} else if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(UtilityModEnchantments.ASHESSENCE.get()) >= 1) {
			if (Mth.nextInt(RandomSource.create(), 1, 3) == 2) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
